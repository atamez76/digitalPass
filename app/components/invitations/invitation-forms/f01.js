...
export default function F01(params) {
  ...
  const newFormDescription = useFormDataStore(
    (state) => state.setNewDescription
  );
  ...
  const debouncedDesc = useDebouncedCallback((value) => {
  ...
  useEffect(() => {
    if (title && host && description && date && time && venue && address && typeOfEvent) {
      setFormIncomplete(false);
    } else {
      setFormIncomplete(true);
    }
  }, [title, description, host, date, time, venue, address, typeOfEvent]);
      ...
      <form className={classes.form} action={formAction}>
        ...
        <div className={classes.form_control}>
          ...
          <textarea
            ...
            defaultValue={
              path.includes("new") ? description : params.template.description
            }
            readOnly={params.readOnly}
            className={params.readOnly ? classes.disabled : ""}
            onChange={(e) => debouncedDesc(e.target.value)}
          />
        </div>
        <div className={classes.form_control}>
        ...
        <div className={classes.form_control}>
          ...
          <textarea
            ...
            name="address"
            defaultValue={
              path.includes("new") ? address : params.template.address
            }
            readOnly={params.readOnly}
            className={params.readOnly ? classes.disabled : ""}
        ...
        <button
          ...
          disabled={path.includes("new") ? false : params.readOnly}
        >
          Change Image
        </button>
        ...
        <div className={classes.form_control}>
          <input id="form" name="form" defaultValue={"F01"} hidden />
          <input
            id="geo"
            name="geo"
          ...
          <input
            ...
            defaultValue={
              selectedimg
                ? selectedimg
                : params.template.styles.background_image
            }
            hidden
          />
          <input
